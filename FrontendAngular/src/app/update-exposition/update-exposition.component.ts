...
export class UpdateExpositionComponent implements OnInit {
  ...
  @Input() ajout!: boolean;
  constructor(private expositionService: ExpositionService) {} // Injectez le service

  ngOnInit(): void {
    if (!this.exposition) {
      this.exposition = new Exposition(); // Assurez-vous que la classe Exposition est importée.
    }
